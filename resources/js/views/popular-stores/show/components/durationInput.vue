<template>
    ...
    <FormItem prop="duration" :error="serverDurationError" class="mb-2">
        <Poptip trigger="click" width="350" placement="top-end" word-wrap class="poptip-w-100"
                content="Enter the total number of days to list this store">
            <div class="d-flex">
                <span class="mr-2" :style="{ minWidth: 'max-content' }">Duration (Days)</span>
                <InputNumber v-model="popularStoreForm.duration" class="w-100"
                             placeholder="100" :disabled="isLoading || (isEditing && !popularStoreForm.reset_dates)"
        ...
        methods: {
            onChange(){
                ...
                this.$nextTick(() => {
                    ...
                    if(this.popularStoreForm.duration === null || this.popularStoreForm.duration === undefined){

                        this.popularStoreForm.duration = this.minimum;
