    ...
    <div>
        ...
        <Modal
            ...
            <div class="mt-4">
                ...
                <Loader v-if="isDeleting" class="mt-2">Removing {{ storeName }}...</Loader>

                <!-- Disclaimer -->
                <Alert v-else type="warning">
                    Remove <span class="font-weight-bold">{{ storeName }}</span>
                    <Divider class="my-2" />
                    <template slot="desc">
                        This popular store will be removed permanently and cannot be recovered again.
                    </template>
                </Alert>
            ...
            <template v-slot:footer>
                <div class="clearfix">
                    <Button type="error" @click.native="attemptPopularStoreDeletion()" class="float-right" :disabled="isDeleting">Remove</Button>
...
<script>
    ...
    export default {
        ...
        computed: {
            storeName(){
                return ((this.popularStore._embedded || {}).store || {}).name;
            },
            popularStoreUrl(){
        ...
        methods: {
            attemptPopularStoreDeletion(){
                ...
                return api.call('delete', this.popularStoreUrl)
                    .then(({data}) => {

                        //  Stop loader
                        self.isDeleting = false;

                        //  Remove the popular Store
                        self.removePopularStore();
            ...
            removePopularStore(){
                ...
                //  Remove popular store from list
                this.popularStores.splice(this.index, 1);

                //  Popular store removed success message
                this.$Message.success({
                    content: 'Popular store removed!',
                    duration: 6
                });
            }
        }
